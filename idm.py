...
def position(y,t,v,a):
    return moving.Point(2000,0.5*a*(t**2)+v*t+y)
...
def s_etoile(a,b,v,DV,T):
    s0=2
    return s0+max(0,v*1+(v*DV)/(2*sqrt(a*b)))

"fonction de maj de l'acceleration"

def acceleration(a,b,v,v0,DV,s,T):
    return a*(1-(v/v0)**4-(s_etoile(a,b,v,DV,T)/s)**2)
    # return decimal.Decimal(a)*(decimal.Decimal(1)-(decimal.Decimal(v)/decimal.Decimal(v0))**delta-(s_etoile(a,v,DV,T)/decimal.Decimal(s))**decimal.Decimal(2))
...
for t in range(1,t_simul):
    acc=0
    speed.append(vitesse(v0,acc))
    # speed.append(vitesse(speed[t-1],acc))
    posV.append(position(posV[t-1].y,1,v0,acc))
